...
def decode_jwt(token: str) -> dict[str, Any]:
    """
    Быстрый разбор токена БЕЗ проверки подписи — только чтобы вытащить
    sub для контекста логов (RequestContextMiddleware). Настоящая
    аутентификация всегда идёт через decode_and_verify_token (JWKS).
    Никогда не использовать этот результат для авторизации. Специально
    не оборачиваю JWTError — вызывающий код (middleware) сам его ловит
    и просто пропускает запрос дальше без user_id в логах.
    """
